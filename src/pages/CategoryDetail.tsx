 ...
 const CategoryDetail = () => {
   ...
   const fetchData = useCallback(async (fetchOffset: number, limit: number) => {
     ...
     try {
       for (const table of config.tables) {
         ...
         const { data, error } = await query.range(fetchOffset, fetchOffset + limit - 1);
   ...
   useEffect(() => {
     ...
     initializeData();
   }, [category, loadInitialData]);
 
   // Real-time subscription for auto-updates without refresh
   useEffect(() => {
     if (!config) return;
     ...
     config.tables.forEach((table: string) => {
       const channel = supabase
         .channel(`${table}_realtime_${category}`)
         .on(
           'postgres_changes',
           ...
           (payload) => {
             ...
             if (payload.eventType === 'INSERT') {
               ...
               if (newItem.approval_status === 'approved' && !newItem.is_hidden) {
                 if (config.tripType && newItem.type !== config.tripType) return;
                 if (config.establishmentType && newItem.establishment_type !== config.establishmentType) return;
                 
                 const itemType = table === 'trips' 
                   ? (newItem.type === 'event' ? 'EVENT' : 'TRIP')
                   : table === 'hotels' ? 'HOTEL' : 'ADVENTURE PLACE';
                 
                 setItems(prev => [{ ...newItem, table, itemType, isOutdated: false }, ...prev]);
               }
