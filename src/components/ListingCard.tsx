...
interface ListingCardProps {
    ...
    onSave?: (id: string, type: string, isCurrentlySaved: boolean) => void; // Updated signature
    isSaved: boolean; // Must be required for local state management to work correctly
    amenities?: string[];
    hidePrice?: boolean;
...
export const ListingCard = ({
    ...
    isCustomDate = false,
    onSave,
    isSaved: initialIsSaved, // Rename prop to avoid conflict with state
    amenities,
    hidePrice = false,
    availableTickets,
    bookedTickets,
    ...
    // 1. Local state to manage the saved status for immediate feedback
    ...
    // 2. Updated save click handler to toggle state and call onSave
    const handleSaveClick = useCallback((e: React.MouseEvent) => {
        e.stopPropagation();
        const newState = !isSaved;
        setIsSaved(newState); // Optimistic UI update
        if (onSave) {
            // Pass the new state to the external handler
            onSave(id, type, isSaved); 
        }
    }, [id, type, isSaved, onSave]);
    ...
    // 3. New custom Blue text class for MapPin (using rgb(0, 80, 255))
    const blueTextClass = "text-[rgb(0,80,255)]";
        ...
        >
                    ...
                    >
                        <Heart
                            className={cn(
                                ...
                                // 4. Use local state for conditional styling
                                isSaved 
            ...
            <div className="p-3 md:p-4 flex flex-col space-y-2">
                ...
                <div className="flex items-center gap-1">
                    {/* 5. MODIFICATION: MapPin Icon Color uses custom Blue (0, 80, 255) */}
                    <MapPin className={cn("h-3 w-3 md:h-4 md:w-4 flex-shrink-0", blueTextClass)} />
                    {/* --- END MODIFICATION --- */}
